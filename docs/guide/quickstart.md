...
屠龙骰（DNDDicer）是专精 **DND5e / DND5r** 跑团的 [NoneBot2](https://nonebot.dev/) 骰娘插件（OneBot V11 适配器），命令手感对齐 [nonebot-dicepp](https://github.com/pear-studio/nonebot-dicepp)（梨骰）。本页带你完成安装、配置与首次使用。

> 规则范围说明：本插件**只做 DND5e/5r**——不做 COC/d100 体系、不做 `.mode` 多规则切换。大成功/大失败等反馈均围绕 **d20** 展开。
...
> 示例中的昵称（艾琳）为发送者的显示名（角色卡名 → 群名片 → QQ 昵称，见[名称显示规则](./overview.md#名称显示规则)）；`=10` 前的 `[5+2]` 为逐骰过程，骰值为本次实际掷出结果。完整掷骰语法见 [掷骰语法](./roll-syntax.md)。

### DM 代操作：用 @ 指定玩家

DM 常需代不在场的玩家结算——把 **@该成员** 写在目标位置即可（@ 也可与名称混写，分隔符 `;` 或空白）：

::: chat
阿岚 | .hp @艾琳 -4d6

注：对艾琳的角色卡造成伤害
屠龙骰 | 艾琳: 当前HP减少[3+2+4]=9
HP:20/30 -> HP:11/30

阿岚 | .ri+3 @艾琳

注：代掷先攻：条目名取角色卡名并与该玩家绑定
屠龙骰 | 艾琳的先攻值是 1D20+3=[10]+3=13
:::

支持 @ 的命令：`.hp`（查看 / 设置 / 伤害 / 治疗 / AOE 混写）、`.长休`、`.ri`、`.init del/first/swap`、`.回合`、`.角色卡` 与 `.状态`（查看他人卡与状态）、检定点命令（`.力量豁免 @玩家`、`.2#敏捷攻击优势 @玩家`）与 `.先攻检定`。

> @ 要写在**目标位置**：`.hp -4d6 @艾琳` 这类游离写法不会执行（骰娘回纠正示例），以免误结算到发送者自己身上。完整说明见 [@ 提及目标](./overview.md#提及目标-dm-代操作)。
