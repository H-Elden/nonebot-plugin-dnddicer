# 战斗轮（.br / .ed / .回合 / .轮次 / .跳过）

战斗轮不是独立系统，而是**先攻表的"进行中状态机"**：`.br` 开局后，先攻列表
变成回合队列，`.ed`（结束回合）自动推进到下一行动者，一轮走完自动进位。
所有命令**仅限群聊**、作用于**本群**先攻表。

## 命令一览

| 命令 | 作用 |
| --- | --- |
| `.br` / `.战斗轮` | 新建战斗轮（**清空本群先攻表**与轮次指针，重新开局） |
| `.回合` / `.轮次` | 查看当前轮与回合 |
| `.回合 +1 / -1 / =2 / 2` | 回合前进/后退/直接跳转（越界自动进位轮次） |
| `.回合 名字` | 按名字跳转到该行动者的回合（支持模糊） |
| `.轮次 +1 / -1 / =3` | 调整轮次 |
| `.ed` / `.结束` | 结束当前回合：播报 + 自动推进下一位 |
| `.跳过 [n]` | 不播报结束直接推进 n 位（默认 1；退场/跳过回合用） |

## 完整时序示例

**① 开局**——先掷先攻入表（详见 [先攻列表](./initiative.md)）：

```text
你发送：.ri
骰娘回复：艾琳的先攻值是 1D20=[15]=15

你发送：.ri20 地精
骰娘回复：地精的先攻值是 20

你发送：.ri 兽人
骰娘回复：兽人的先攻值是 1D20=[18]=18
```

**② 开始战斗轮**：

```text
你发送：.br
骰娘回复：已创建新战斗轮。清除先攻表、当前回合。
```

> ⚠️ `.br` 会**清空本群先攻表**重新开局——战斗中误发请用 `.init` 检查并重新入表。

**③ 查看当前回合**：

```text
你发送：.回合
骰娘回复：现在是第1轮第1回合，地精的回合。
```

**④ 结束回合自动推进**（`.ed`）：走完一轮自动进入下一轮并播报：

```text
你发送：.ed
骰娘回复：地精的回合结束了。
现在是兽人的回合。

你发送：.ed
骰娘回复：兽人的回合结束了。
新的一轮，现在是第2轮。
现在是艾琳的回合。请玩家 @艾琳 开始行动。   ← 轮到绑定 QQ 的玩家时输出 @ 提醒
```

**⑤ 手动调整回合/轮次**（DM 修正）：

```text
你发送：.回合-1
骰娘回复：现在是兽人的回合。          ← 回到上一行动者

你发送：.回合 哥布林                   ← 按名字跳转（精确→模糊）
骰娘回复：现在是哥布林的回合。

你发送：.轮次+1
骰娘回复：现在变成第2轮了。
现在是兽人的回合。
```

越界自动进位：在第 3 回合（共 3 人）发 `.回合+1` → `新的一轮，现在是第2轮。\n现在是兽人的回合。`

**⑥ 跳过回合**（NPC 退场等，静默推进）：

```text
你发送：.跳过
骰娘回复：现在是地精的回合。           ← 跳过当前行动者（不播报"回合结束"）

你发送：.跳过 2                        ← 一次推进多位，可能直接跨轮
骰娘回复：新的一轮，现在是第2轮。
现在是兽人的回合。
```

**⑦ 结束战斗**（无需专门命令）：不再推进即可；下次 `.br` 重新开局。

## 常见提示

| 场景 | 回复 |
| --- | --- |
| 先攻表为空时操作战斗轮 | `目前先攻列表为空，故不存在回合与轮次。` |
| `.回合 名字` 无匹配 / 多匹配 | `没有找到这个回合。` / `找到复数回合，请换一个关键词。` |
| `.回合=9`（超出总人数） / `.回合=0` | `这个数字太大了。` / `这个数字太小了。` |
| `.回合+abc`（非数字） | `这不是数字。` |

## 一期范围说明

- **BUFF 计时表不做**（本轮中自动到期的 BUFF 由 DM 手动管理）；
- NPC 先攻条目不关联临时 HP；实体显示入表时快照的名称（离线可用）；
- `.跳过` 为本插件按"跳过当前单位"语义自研（DicePP 无此命令）。
