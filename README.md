...
>
> 当前版本：**v0.3.0** · 完整使用手册见 **📖 [使用文档](https://h-elden.github.io/nonebot-plugin-dnddicer/)**
...
| 命令 | 说明 |
| --- | --- |
| `.查询 <关键词>` / `.搜索 <关键词>` | 按名称 / 全文检索《5e不全书》；多关键词用空格分隔表示同时满足、`\|` 表示或；返回候选后回数字看词条、`+` / `-` 翻页（60 秒内有效、只认发起者本人） |
...
> 规则查询默认关闭（`dnddicer_query_enabled`）：开启后群聊仍受本群服务开关管辖，私聊可用；查询内容来自《5e不全书》，只用于当次回复与短时内存缓存、不落盘、不随插件分发。详见[规则查询](./docs/guide/query.md)。
...
6. ⏳ **第二期 T2**：牌堆/随机表、规则查询；
7. ⏳（可选）**T3 增强期**：法术位管理（`.ss/.cast`）、死亡豁免闭环（`.ds`）、BUFF/临时加值表（`.buff`）等。
