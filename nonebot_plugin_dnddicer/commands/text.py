...
TXT_QUERY_DISABLED = "规则查询功能未开启，请联系骰主解决。"
...
TXT_QUERY_IMAGE_FALLBACK = "（图片模式未就绪，请联系骰主解决）"

# ── 规则查询图片模式：按处开关 .查询图片（2026-09-25 新增）──────────────
# 语义：骰主总开关只决定「能不能用图片」，各群/私聊默认文字、需主动开启。
# {where} 为「本群」（群聊）或「你」（私聊），由命令层按事件类型填充。
# 面向群成员一律「请联系骰主解决」，不带 .env 配置名与安装命令。
TXT_QUERY_IMAGE_USAGE = "用法：.查询图片 on / off（无参数查看当前设置）"
TXT_QUERY_IMAGE_BAD_ARG = "无效参数：{arg}。{usage}"
TXT_QUERY_IMAGE_STATE_OFF = (
    "{where}的查询结果当前以文字显示。发送 .查询图片 on 可改为图片。"
)
TXT_QUERY_IMAGE_STATE_ON = (
    "{where}的查询结果当前以图片显示。发送 .查询图片 off 可改回文字。"
)
#: 本处已设为图片，但骰主当前未开放（总开关关或依赖缺失）→ 暂以文字显示
TXT_QUERY_IMAGE_STATE_PENDING = (
    "{where}已设为图片显示，但骰主当前未开放，暂以文字显示。"
)
TXT_QUERY_IMAGE_ON = "已开启：{where}的查询结果将以图片显示。"
TXT_QUERY_IMAGE_OFF = "已关闭：{where}的查询结果将以文字显示。"
#: 骰主未开启图片模式：开启请求被拒（不写入设置，仍以文字显示）
TXT_QUERY_IMAGE_DISABLED = (
    "无法开启：骰主未开启图片模式，请联系骰主解决。当前仍以文字显示。"
)
#: 总开关已开但渲染依赖未装 / 加载失败
TXT_QUERY_IMAGE_NOT_READY = (
...
#: {hint} 按命令区分——`.查询` 附一句建议改用 `.搜索` 全文检索；`.搜索` 本身
#: 就是全文检索，不再建议自己（传空串）
TXT_QUERY_NO_RESULT_SCOPED = (
    "没有找到与「{keyword}」相关的内容。"
    "{where}的查询范围为 {scope}，可发送 .查询范围 全部 放开全部书目后再试。{hint}"
)
#: `.查询` 时的建议句（见上；`.搜索` 时传空串）
TXT_QUERY_NO_RESULT_SCOPED_HINT = "或者尝试使用 .搜索 命令来进行全文检索。"
#: .规则书 文字形态的标题与分组行
