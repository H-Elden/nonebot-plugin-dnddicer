...
- 攻击检定点已退役（2026-09-24）：攻击与伤害一律走 `$武器$` 项，卡面不再登记
  攻击熟练与攻击额外加值（魔法武器的加值直接写进武器项的命中/伤害里）。
"""
...
#: 「骷髅」是哨塔里的亡灵守卫，用于承伤特性（易伤）示例；第二战：石桥桥头的
#: 「木乃伊」用于隐藏血量与抗性/易伤示例）
NPCS = ["哥布林", "熊地精", "狼", "向导", "骷髅", "木乃伊"]

# ── 四张角色卡（5r / 2024 玩家手册；`.角色卡记录` 负载，不含命令名）──────────
#: 武器数值口径：命中加值 = 攻击属性调整 + 熟练加值（+ 魔法加值）；伤害加值 =
#: 攻击属性调整（+ 魔法加值）。薇拉的长剑 +1（命中与伤害各含 1）、洛恩的法术
#: 攻击 +7（智力 +4、熟练 +3）、塔莉的灵巧武器走敏捷（+4）并有 3d6 偷袭骰。
CARDS: Dict[str, str] = {
    "薇拉": "\n".join(
        [
            "$姓名$ 薇拉",
